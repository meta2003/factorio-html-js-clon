// Loads build/Factio.html under Node with minimal DOM/canvas stubs and returns the game's
// global namespace. Shared by test/headless.js (scenario runner) and test/playthrough.js (bot).
...
const DEFAULT_HTML = path.resolve(__dirname, '../../build/Factio.html');

// loadGame(htmlPath?) -> { F, window }. Throws on a load error.
function loadGame(htmlPath) {
  const html = fs.readFileSync(htmlPath || DEFAULT_HTML, 'utf8');
  const scripts = [...html.matchAll(/<script[^>]*>([\s\S]*?)<\/script>/gi)].map(m => m[1]);
  if (!scripts.length) throw new Error('no <script> blocks found in ' + (htmlPath || DEFAULT_HTML));
  ...
  } catch (e) { throw new Error('LOAD ERROR: ' + ((e && e.stack) || e)); }
  const F = window.F;
  if (!F) throw new Error('window.F namespace missing');
  for (const fn of ['newGame', 'tick']) if (typeof F[fn] !== 'function') throw new Error('F.' + fn + ' is not a function');
  return { F, window };
}

module.exports = { loadGame, DEFAULT_HTML };
